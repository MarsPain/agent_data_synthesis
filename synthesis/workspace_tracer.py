...
def _validate_proof_root(
    ...
    if subject.get("evidence_class") != "deterministic_offline":
...
def _verify_positive_chain(
    *,
    ...
    if provider.get("evidence_class") != "deterministic_offline":
        raise WorkspaceTracerProofError("provider_evidence_class_mismatch")
    if not provider.get("attempts") or any(
        ...
        raise WorkspaceTracerProofError("provider_contract")
    try:
        replayed_attempt_count = _replay_provider_attempts(
            root_dir=root_dir,
            artifacts=artifacts,
            anchors=anchors,
            plan=plan,
    ...
    if replayed_attempt_count != len(provider["attempts"]):
        raise WorkspaceTracerProofError("provider_contract")
